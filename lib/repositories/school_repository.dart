...
class SchoolRepository {
  ...
  Future<ApiResponse<List<ClassModel>>> getClasses() async {
    try {
      ...
      return ApiResponse<List<ClassModel>>.fromJson(response.data, (json) {
  ...
  Future<ApiResponse<List<TeamMemberInfo>>> searchStudents({
    ...
    try {
      final response = await _client.dio.get(
        '/api/students/search',
        queryParameters: {'classId': classId, 'query': query},
      );

      return ApiResponse<List<TeamMemberInfo>>.fromJson(response.data, (json) {
        final list = json as List<dynamic>;
        return list
