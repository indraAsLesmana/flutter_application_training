  ...
  void _submit() async {
    ...
    try {
      final success = await taskProvider.createNewTask(
        ...
        attachmentUrl: _attachmentUrlController.text.isEmpty
            ? null
      ...
      if (mounted) {
        setState(() {
          if (e is DioException) {
            _formError = DioClient.getErrorMessage(e);
          } else {
            final msg = e.toString().replaceFirst('Exception: ', '');
            _formError = msg.startsWith('Terjadi kesalahan:')
                ? msg
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                decoration: BoxDecoration(
                  border: Border.all(
                    color: _endDate == null && _formError != null
                        ? Colors.red
                        : Colors.grey[400]!,
                  ),
                  borderRadius: BorderRadius.circular(8),
                ...
                child: ListTile(
                  title: Text(
                    _endDate == null
                        ? 'Pilih Tenggat Waktu'
                        : 'Tenggat: ${_endDate.toString().split(' ')[0]}',
                  ),
                  trailing: const Icon(Icons.calendar_today),
                  onTap: () async {
                    final date = await showDatePicker(
              ...
              SwitchListTile(
                ...
                subtitle: const Text(
                  'Izinkan siswa menambahkan anggota kelompok saat mengumpulkan',
                ),
